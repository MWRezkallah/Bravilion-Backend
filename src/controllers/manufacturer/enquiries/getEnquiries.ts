...
export const getEnquiry = async (req: Request, res: Response) =>{
    ...
    try {
        ...
        const enquiries:any = await manuRepo.collection?.find({...query,"enquiries.enquiryId":enquiryID}, {projection:{"manufacturerId":"$_id","_id":0, "enquiries":{$elemMatch:{"enquiryId":enquiryID}}}}).toArray()
        res.status(200).send({
            status:"success",
            data: enquiries[0]?.enquiries || []
        })
